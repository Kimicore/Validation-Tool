...
def generate_default_output_path():
    """Build an auto-incrementing, timestamped default output filename."""

    output_dir = BASE_DIR / "output"
    output_dir.mkdir(parents=True, exist_ok=True)

    existing = list(output_dir.glob("validation_report_*.json"))
    next_number = len(existing) + 1

    timestamp = datetime.now().strftime("%Y-%m-%d_%H%M%S")

    return output_dir / f"validation_report_{next_number}_{timestamp}.json"
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to write the validation report (default: auto-generated, "
             "output/validation_report_<N>_<timestamp>.json)"
    )
